  ...
  */
  ...
  void loop()
  {
        ...
        if (millis() - oldTime > MINIMUM_SERVO_UPDATE_TIME)
        {
              ...
              if(abs(newDegree-oldDegrees) > GRIPPER_MINIMUM_STEP)      //Angle does not uupdate if the pressure sensor is engaged
              //if(abs(newDegree-oldDegrees) > GRIPPER_MINIMUM_STEP)      //Angle does not uupdate if the pressure sensor is engaged
...
//Stream the data from the EMG and force sensor acquisition, added by AMK, 2/8/22
        ...
        Serial.print(" ");
        Serial.println(analogReadings*5.0/1024);    //Voltage from EMG
}
